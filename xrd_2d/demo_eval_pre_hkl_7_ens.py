...
argparser.add_argument('--test_path', type=str, default='/scratch/zzh136/xrd_2d/re_exp/test/zone20', help='model name')
...
class ensemble_model(nn.Module):
    ...
    def forward(self, x, h, k, l):
        ...
        for model in self.models:
            output, output7, ph, pk, pl = model(x, h, k, l)
            outputs.append(output)
            outputs7.append(output7)
            ph_logit.append(ph)
            pk_logit.append(pk)
            pl_logit.append(pl)
        outputs = torch.stack(outputs, dim=0)
        outputs7 = torch.stack(outputs7, dim=0)
        ph_logit = torch.stack(ph_logit, dim=0)
        pk_logit = torch.stack(pk_logit, dim=0)
        pl_logit = torch.stack(pl_logit, dim=0)
        outputs = torch.mean(outputs, dim=0)
        outputs7 = torch.mean(outputs7, dim=0)
        ph_logit = torch.mean(ph_logit, dim=0)
        pk_logit = torch.mean(pk_logit, dim=0)
        pl_logit = torch.mean(pl_logit, dim=0)
        return outputs, outputs7, ph_logit, pk_logit, pl_logit
...
if __name__ == "__main__":
    ...
    ckpt_lists = ["/scratch/zzh136/xrd_2d/axis20_data/ckpt/pretrained_vgg19_hkl_noflip_7.pth"]
                  # "/scratch/zzh136/xrd_2d/model_/model2/ckpt/model_2_pretrained_vgg19_hkl_noflip_7.pth", 
                  # "/scratch/zzh136/xrd_2d/model_/model3/ckpt/model_3_pretrained_vgg19_hkl_noflip_7.pth"]
    
    
    ens_model_lists = []
    
    for i in range(len(ckpt_lists)):
        ckpt_lists[i] = torch.load(ckpt_lists[i])
    
        uni_model = models.vgg19(pretrained=False, num_classes=230)
        optimazier = optim.SGD(uni_model.parameters(), lr=0.001, momentum=0.9)
        criterion = nn.CrossEntropyLoss()

        device =  torch.device("cuda:0" if torch.cuda.is_available() else "cpu")


        uni_model=HKL_model(uni_model)
        uni_model.load_state_dict(ckpt_lists[i])
        uni_model = uni_model.to(device)
        uni_model.eval()
        ens_model_lists.append(uni_model)
    epochs = 100
    batch_size = 128
    # train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=4)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=True, num_workers=4)
    
    model = ensemble_model(ens_model_lists)
